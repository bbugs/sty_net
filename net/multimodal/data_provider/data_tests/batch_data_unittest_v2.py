...
print "y_local shape (n_regions x n_unique_words)", batch_data.y_local.shape
...
assert np.allclose(batch_data.y_local[5:10, one_shoulder_index], 1)  # second img regions
assert np.allclose(batch_data.y_local[0:5, one_shoulder_index], -1)  # first img regions
assert np.allclose(batch_data.y_local[10:15, one_shoulder_index], -1) # third img regions


v_neck_index = batch_data.unique_words_list.index("v-neck")
assert np.allclose(batch_data.y_local[0:5, v_neck_index], -1)  # second img regions
assert np.allclose(batch_data.y_local[5:10, v_neck_index], -1)  # first img regions
assert np.allclose(batch_data.y_local[10:15, v_neck_index], 1)  # third img regions
